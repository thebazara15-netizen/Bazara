        ...
        <div className={mobile ? "grid gap-6" : "grid grid-cols-[18rem_1fr] overflow-hidden rounded-2xl border border-slate-200"}>
          ...
          <div className={mobile ? "" : "p-7"}>
            ...
            <div className="mt-7 grid gap-5 border-t border-slate-200 pt-6 sm:grid-cols-3">
              ...
              <div><h4 className="text-sm font-bold text-slate-950">Marketplace services</h4><div className="mt-3 grid gap-2 text-sm text-slate-600"><a onClick={onClose} href="/#visual-search" className="hover:text-orange-700">Visual search preview</a>{roleLink && <Link onClick={onClose} href={roleLink.href} className="hover:text-orange-700">{roleLink.label}</Link>}</div></div>
            </div>
          </div>
        </div>
      </div>
