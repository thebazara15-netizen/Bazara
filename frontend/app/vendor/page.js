...
export default function VendorDashboard() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black text-white p-4 md:p-8">
      ...
      {/* My Products Section */}
      <div className="bg-gradient-to-br from-gray-800/40 to-gray-900/40 backdrop-blur-md rounded-2xl p-4 md:p-8 border border-gray-700/50 shadow-2xl">
        <div className="flex items-center gap-3 mb-6 md:mb-8">
          <div className="text-2xl md:text-3xl">🏪</div>
          <h2 className="text-lg md:text-2xl font-bold bg-gradient-to-r from-blue-400 to-blue-600 bg-clip-text text-transparent">
            ...
            {products.map(product => (
              ...
              >
                ...
                {product.images && product.images.length > 0 ? (
                  <div className="relative h-32 sm:h-40 md:h-48 bg-gray-900 overflow-hidden">
                    ...
                    <div className="absolute top-2 right-2 z-30">
                      <button
                        ...
                        className="bg-black/60 hover:bg-black/80 text-white p-2 rounded-lg transition text-lg"
                      >
                        ⋯
                      </button>
                      ...
                      {openMenuId === product.id && (
                        <div className="absolute right-0 top-full mt-2 bg-gray-800 border border-gray-700 rounded-lg shadow-xl z-50 w-48">
                          <button
                            onClick={() => {
                              deleteProduct(product.id);
                              setOpenMenuId(null);
                            }}
                            className="w-full text-left px-4 py-3 hover:bg-red-600/20 text-red-400 hover:text-red-300 font-medium transition text-sm flex items-center gap-2"
                          >
                            🗑️ Delete Product
                          </button>
                        </div>
                    ...
                    {product.images.length > 1 && (
                      <>
                        <button
                          ...
                          className="absolute left-1 sm:left-2 top-1/2 transform -translate-y-1/2 bg-black/70 hover:bg-black/90 text-white px-2 sm:px-3 py-1 sm:py-2 rounded-lg opacity-0 group-hover:opacity-100 transition text-xs sm:text-base"
                        >
                          ◀
                        </button>
                        <button
                          onClick={() => nextImage(product.id)}
                          className="absolute right-1 sm:right-2 top-1/2 transform -translate-y-1/2 bg-black/70 hover:bg-black/90 text-white px-2 sm:px-3 py-1 sm:py-2 rounded-lg opacity-0 group-hover:opacity-100 transition text-xs sm:text-base"
                        >
                          ▶
                        </button>
                      </>
                  ...
                  <div className="w-full h-32 sm:h-40 md:h-48 bg-gradient-to-br from-gray-700 to-gray-800 flex items-center justify-center">
                ...
                <div className="p-3 md:p-5">
                  <h3 className="font-bold text-sm md:text-lg mb-2 text-white line-clamp-2 group-hover:text-orange-400 transition">
                    {product.name}
                  </h3>
                  <p className="text-gray-400 text-xs md:text-sm mb-2 md:mb-3 line-clamp-2">{product.description}</p>
                  
                  {/* Price */}
                  <div className="mb-2 md:mb-3 p-2 md:p-3 bg-gradient-to-r from-orange-600/20 to-orange-700/20 border border-orange-500/30 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div className="bg-gray-700/50 p-2 rounded-lg">
                      ...
                      <p className="text-white font-semibold text-sm">{product.moq} units</p>
                    </div>
                    <div className="bg-gray-700/50 p-2 rounded-lg">
                      <p className="text-gray-500 text-xs">Stock</p>
                      <p className="text-white font-semibold text-sm">{product.stock} units</p>
                    </div>
                  </div>
                </div>
              </div>
