...
export default function AdminDashboard() {
    ...
    return users
      .filter((user) => {
        ...
        const matchesFilter =
          ...
          (userFilter === "verified-vendors" && user.role === "VENDOR" && user.isVerified) ||
          (userFilter === "pending-vendors" && user.role === "VENDOR" && !user.isVerified);

        return matchesSearch && matchesFilter;
      })
      .sort((a, b) => {
        if (userSort === "oldest") return getTime(a.createdAt) - getTime(b.createdAt);
        if (userSort === "email") return (a.email || "").localeCompare(b.email || "");
        if (userSort === "role") return (a.role || "").localeCompare(b.role || "");
        return getTime(b.createdAt) - getTime(a.createdAt);
      });
  }, [userFilter, userSearch, userSort, users]);
  ...
  const visibleProducts = useMemo(() => {
    ...
    return products
      ...
      .sort((a, b) => {
        if (productSort === "oldest") return getTime(a.createdAt) - getTime(b.createdAt);
        if (productSort === "amount-high") return Number(b.finalPrice || b.basePrice || 0) - Number(a.finalPrice || a.basePrice || 0);
        if (productSort === "amount-low") return Number(a.finalPrice || a.basePrice || 0) - Number(b.finalPrice || b.basePrice || 0);
        if (productSort === "stock-high") return Number(b.stock || 0) - Number(a.stock || 0);
        if (productSort === "name") return (a.name || "").localeCompare(b.name || "");
        return getTime(b.createdAt) - getTime(a.createdAt);
      });
  }, [productFilter, productSearch, productSort, products]);
  ...
  const visibleOrders = useMemo(() => {
    ...
    return orders
      .filter((order) => {
        ...
        const matchesFilter =
          orderFilter === "all" || String(order.status || "").toUpperCase() === orderFilter;

        return matchesSearch && matchesFilter;
      })
      .sort((a, b) => {
        if (orderSort === "oldest") return getTime(a.createdAt) - getTime(b.createdAt);
        if (orderSort === "amount-high") return Number(b.totalAmount || 0) - Number(a.totalAmount || 0);
        if (orderSort === "amount-low") return Number(a.totalAmount || 0) - Number(b.totalAmount || 0);
            ...
            {users.length > 0 && (
              <div className="mb-4 grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                >
                  ...
                  <option value="verified-vendors">Verified vendors</option>
                ...
                {visibleOrders.map(order => (
                  <div key={order.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between bg-gray-900/50 p-3 md:p-4 rounded-lg border border-gray-700 hover:border-gray-600 hover:bg-gray-800/50 transition-all duration-200 gap-2 sm:gap-4">
                    ...
                    <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap ${
                      ...
                      String(order.status || "").toUpperCase() === "PENDING" ? "bg-yellow-500/20 text-yellow-300" :
                      String(order.status || "").toUpperCase() === "SHIPPED" ? "bg-purple-500/20 text-purple-300" :
                      "bg-blue-500/20 text-blue-300"
                    }`}>
