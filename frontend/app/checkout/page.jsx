...
function DraftReview({ draft, onChange }) {
  const labels = { SELLER_SHIPPING_POLICY_REQUIRED: "Seller shipping policy not configured", SELLER_TAX_PROFILE_REQUIRED: "Seller GST/state information required", PRODUCT_TAX_CONFIGURATION_REQUIRED: "Product GST information required", TAX_INCLUSIVE_PRODUCT_UNSUPPORTED: "Tax-inclusive product pricing requires review" };
  return <div className="rounded-2xl border border-green-200 bg-white p-6"><p className="text-sm font-bold uppercase tracking-wider text-green-700">Draft created · {draft.pricingStatus?.replaceAll("_", " ")}</p><h2 className="mt-2 text-2xl font-extrabold">Supplier pricing breakdown</h2><p className="mt-2 text-sm text-gray-600">Draft #{draft.draftId} · Expires {new Date(draft.expiresAt).toLocaleString("en-IN")}</p>{draft.stale && <p className="mt-4 rounded-xl bg-amber-50 p-4 font-semibold text-amber-800">This draft is stale or expired. Create a new draft.</p>}{draft.missingRequirements?.length > 0 && <div className="mt-4 rounded-xl bg-amber-50 p-4 text-sm text-amber-900"><strong>Pricing is not complete:</strong><ul className="mt-2 list-disc space-y-1 pl-5">{draft.missingRequirements.map((item, index) => <li key={`${item.code}-${item.vendorId}-${item.productId}-${index}`}>{labels[item.code] || "Pricing configuration required"}{item.productId ? ` (product ${item.productId})` : ""}</li>)}</ul></div>}<div className="mt-6 space-y-4">{draft.sellerGroups?.map((group) => <div key={group.vendorId} className="rounded-xl border border-gray-200 p-4"><p className="font-bold">{group.sellerSnapshot?.displayName || `Seller ${group.vendorId}`}</p>{group.items?.map((item) => <div key={item.productId} className="mt-3 flex justify-between gap-4 text-sm"><span>{item.productName} × {item.quantity}</span><span className="font-bold">{money(item.lineSubtotalPaise)}</span></div>)}{group.shippingStatus === "QUOTE_REQUIRED" && <p className="mt-4 font-bold text-amber-700">Freight quote required for this supplier</p>}<dl className="mt-4 grid grid-cols-2 gap-2 border-t pt-4 text-sm"><dt>Subtotal</dt><dd className="text-right font-bold">{money(group.subtotalPaise)}</dd><dt>Shipping</dt><dd className="text-right font-bold">{group.shippingStatus === "QUOTE_REQUIRED" ? "Freight quote required" : money(group.shippingPaise)}</dd><dt>GST</dt><dd className="text-right font-bold">{money(group.taxPaise)}</dd><dt>Seller total</dt><dd className="text-right font-bold">{money(group.grandTotalPaise)}</dd></dl></div>)}</div><button onClick={onChange} className="mt-6 text-sm font-bold text-orange-600">Change addresses and create a new draft</button></div>;
}
