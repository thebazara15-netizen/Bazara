...
export default function AccountPage() {
  ...
  useEffect(()=>{const params=new URLSearchParams(window.location.search);if(params.get("section")==="inbox"){setActive("inbox");setInitialConversationId(params.get("conversation"));}const current=getToken();const user=current?decodeToken(current):null;if(!current){localStorage.setItem("redirect","/account");router.push("/login");return;}if(user?.role==="VENDOR"){router.push("/vendor");return;}if(user?.role==="ADMIN"){router.push("/admin");return;}if(user?.role!=="CLIENT"){router.push("/");return;}setToken(current);},[router]);
  ...
  const openInquiryInbox=async(id)=>{try{const conversation=await request(`/inbox/inquiries/${id}/open`,{method:"POST"});setInitialConversationId(conversation.conversationId);setActive("inbox");router.replace(`/account?section=inbox&conversation=${conversation.conversationId}`);}catch(error){setFeedback({type:"error",message:error.message});}};
  const cartProducts=new Set(cart.map((item)=>Number(item.productId)));
  const cartCount=cart.reduce((sum,item)=>sum+Number(item.quantity||0),0);
  const content=active==="inbox"?<InboxPanel token={token} role="CLIENT" initialConversationId={initialConversationId}/>:active==="overview"?<AccountOverview counts={{wishlist:wishlistItems.length,cart:cartCount,rfqs:rfqs.length,inquiries:inquiries.length}} onNavigate={setActive}/>:active==="profile"?<AccountProfile key={profile?.id} profile={profile} saving={saving} onSave={saveProfile}/>:active==="addresses"?<AccountAddresses addresses={addresses} saving={saving} onCreate={createAddress} onUpdate={updateAddress} onDelete={deleteAddress}/>:active==="wishlist"?<AccountWishlist items={wishlistItems} cartProducts={cartProducts} onAddToCart={addToCart}/>:active==="rfqs"?<AccountRfqs rfqs={rfqs} decisionId={decisionId} onDecision={decideQuote}/>:active==="inquiries"?<AccountInquiries inquiries={inquiries} onOpenInbox={openInquiryInbox}/>:<AccountCart items={cart}/>;
