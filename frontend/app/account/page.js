...
export default function AccountPage() {
  ...
  const load=useCallback(async()=>{if(!token)return;try{const [profileData,cartData,rfqData,inquiryData]=await Promise.all([request("/account/profile"),request("/cart"),request("/rfqs/my"),request("/inquiries/my")]);setProfile(profileData);setCart(Array.isArray(cartData)?cartData:[]);setRfqs(Array.isArray(rfqData)?rfqData:[]);setInquiries(Array.isArray(inquiryData)?inquiryData:[]);}catch(error){setFeedback({type:"error",message:error.message});}finally{setLoading(false);}},[request,token]);
